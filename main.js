...
async function searchReverseTransaction() { 
  ...
  try {
    ...
    if (result.status === 'SUCCESS') { 
      const matchData = result.data; // GASから返された一致データ
      searchResultContent.innerHTML = `
        <div class="search-match">
          ...
          <div class="match-details">
            <div class="match-details-row">
              ...
              <span class="match-details-value">${matchData.date || 'N/A'}</span>
            </div>
            <div class="match-details-row">
              <span class="match-details-label">👤 入力者:</span>
              <span class="match-details-value">${matchData.name || 'N/A'}</span>
            </div>
            <div class="match-details-row">
              <span class="match-details-label">📤 貸主:</span>
              <span class="match-details-value">${matchData.lender || 'N/A'}</span>
            </div>
            <div class="match-details-row">
              <span class="match-details-label">📥 借主:</span>
              <span class="match-details-value">${matchData.borrower || 'N/A'}</span>
            </div>
            <div class="match-details-row">
              <span class="match-details-label">🏷️ カテゴリー:</span>
              <span class="match-details-value">${matchData.category || 'N/A'}</span>
            </div>
            <div class="match-details-row">
              <span class="match-details-label">📝 品目:</span>
              <span class="match-details-value">${matchData.item || 'N/A'}</span>
            </div>
            <div class="match-details-row">
              <span class="match-details-label">💵 金額:</span>
              <span class="match-details-value">¥${(parseInt(matchData.amount) || 0).toLocaleString('ja-JP')}</span>
            </div>
            <div class="match-details-row">
              <span class="match-details-label">⏰ 入力日時:</span>
              <span class="match-details-value">${matchData.inputDate || 'N/A'}</span>
            </div>
...
async function submitData(options = {}) {
  ...
  try {
    ...
    if (!response.ok) {
        // GASからのエラーメッセージを解析できる場合はそれも表示
        const errorText = await response.text();
        try {
            const errorJson = JSON.parse(errorText);
            throw new Error(`GASエラー: ${response.status} ${response.statusText} - ${errorJson.message || '不明なエラー'}`);
        } catch (jsonError) {
            throw new Error(`GASエラー: ${response.status} ${response.statusText} - ${errorText || 'レスポンス内容なし'}`);
        }
    }
