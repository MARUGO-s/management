...
function displayOriginalData() {
  ...
  const formattedUnitPrice = originalData.unitPrice && !isNaN(parseInt(String(originalData.unitPrice).replace(/[,¥]/g, ''))) ?
    `¥${parseInt(String(originalData.unitPrice).replace(/[,¥]/g, '')).toLocaleString('ja-JP')}` :
    (originalData.unitPrice || '---');

  // 全てのデータ項目を表示するようにHTMLを更新
