...
document.getElementById("login-button").addEventListener("click", function() {
    ...
    class PasswordAuth {
        ...
        handlePasswordSubmit() {
            ...
            // パスワードチェック（少し遅延を入れてセキュリティらしく見せる）
            setTimeout(() => {
                // ここを修正: obfuscated.js から提供される verifyPassword 関数を呼び出す
                // verifyPassword 関数が利用可能かチェック
    ...
    class GoogleSheetsAPIAnalyzer {
        constructor() {
            ...
            this.apiKey = 'AIzaSyAcy7hXztCjngG2poP-Sn_g6ED0TESnNTE'; // 実際のAPIキーに置き換えてください
            this.spreadsheetId = '1Z1i7p1s5GeXdfhMoSrcu-JzJL_yima7FNHCoJ7Fz4iY'; // 実際のスプレッドシートIDに置き換えてください
        ...
        async init() {
            ...
            // APIキーが設定されている場合は自動接続
            if (this.apiKey && this.apiKey !== 'YOUR_API_KEY_HERE') { // APIキーが空でないこともチェック
                await this.connectToGoogleSheets();
            } else {
                this.log('APIキーを設定してください', 'error');
            }
        }
        ...
        async connectToGoogleSheets() {
            if (!this.apiKey || this.apiKey === 'YOUR_API_KEY_HERE') {
                this.log('APIキーが設定されていません', 'error');
                return;
            }
            ...
            try {
                ...
                this.debugLog('メタデータ取得 URL', metadataUrl);
                
                const metadataResponse = await fetch(metadataUrl);
                
                if (!metadataResponse.ok) {
                    throw new Error(`メタデータ取得失敗: ${metadataResponse.status} ${metadataResponse.statusText}`);
                }
                
                const metadata = await metadataResponse.json();
                this.debugLog('スプレッドシート情報', metadata);
                ...
                this.debugLog('対象シートが見つかりました', targetSheetName);
                
                // Google Sheets API v4 を使用してデータを取得
                const range = `${targetSheetName}!A:I`; // A列からI列までを取得
                const url = `https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}/values/${range}?key=${this.apiKey}`;
                
                this.debugLog('API リクエスト URL', url);
                
                const response = await fetch(url);
                
                this.debugLog('API レスポンス', {
                ...
                if (!response.ok) {
                    if (response.status === 403) {
                        throw new Error('APIキーが無効または権限が不足しています。APIキーとスプレッドシートの共有設定を確認してください。');
        ...
        async processSheetData(values) {
            ...
            const nameIndex = getColumnIndex(['名前', 'name', '取引名']); // B列
            ...
            const amountIndex = getColumnIndex(['金額', 'amount']); // G列
            const inputDateIndex = getColumnIndex(['入力日時', 'inputDate']); // H列
            const correctionIndex = getColumnIndex(['修正', 'correction']); // I列
        ...
        searchCorrections() {
            ...
            try {
                ...
                let results = this.data.filter(row => {
                    // 修正列に「✏️修正」があるかチェック (J列またはI列に相当)
                    const isCorrectionMarked = row.correction && row.correction.includes('✏️修正');

                    if (!isCorrectionMarked) return false; // 修正マークがない場合は除外
                    ...
                    // 店舗名による絞り込み
                    if (storeName) {
                        // 店舗名が指定されている場合、その店舗が貸主または借主である修正データのみ
                        return row.lender === storeName || row.borrower === storeName;
                    } else {
                        // 店舗名が指定されていない場合、全ての修正データを表示
                        return true;
                    }
                });
                ...
                // 店舗名が指定されているかどうかに関わらず、サマリーは表示しない
                hideSummary(); // <-- ここでサマリーを非表示にする関数を呼び出す
                this.displayAllDataResults(displayTitle); // 詳細テーブルのみ更新するためにこの関数を使用
        ...
        displayResults(storeName) {
            ...
            document.getElementById('details-subtitle').textContent = '期間内の取引データ（各列をクリックでソート）';
        ...
        displayAllDataResults(titleText = "全取引詳細") { // タイトルを引数で受け取る
            ...
            document.getElementById('details-subtitle').textContent = '期間内の取引データ（各列をクリックでソート）'; // 全データ表示でもこのサブタイトルを使う
        ...
        updateDetailsTable(isAllData = false) {
            ...
            if (this.searchResults.length === 0) {
                detailsTable.innerHTML = '<tr><td colspan="8" class="no-results">検索条件に該当するデータがありません</td></tr>';
                return;
            }
...
💵 金額: ${formattedAmount}
スプレッドシート行: ${selectedData.originalRowIndex || '不明'}
...
続行しますか？`;
        ...
        redirectToCorrectionPage(selectedData) {
            try {
                ...
                // 必要なフィールドの検証
                const requiredFields = ['date', 'lender', 'borrower', 'category', 'item', 'amount', 'originalRowIndex']; // originalRowIndex を追加
                const missingFields = requiredFields.filter(field => !selectedData[field]);
                if (missingFields.length > 0) {
                    console.warn('不足しているフィールド:', missingFields);
                }
                ...
                if (!savedData) {
                    throw new Error('sessionStorageへの保存に失敗しました');
                }
                ...
                // バックアップとして、localStorageにも保存
                try {
                    localStorage.setItem('correctionData', dataToSave);
                    console.log('localStorageにもバックアップ保存完了');
                } catch (localStorageError) {
                    console.warn('localStorageへのバックアップ保存に失敗:', localStorageError);
                }
                ...
                // alert(`修正ページへの移動中にエラーが発生しました: ${error.message}`); // alert の代わりにカスタムモーダルを使用
                ...
                console.log('- localStorage状態:', Object.keys(localStorage));
            }
        }

        exportResults() {
            if (this.searchResults.length === 0) {
                // alert('エクスポートするデータがありません'); // alert の代わりにカスタムモーダルを使用
                this.showCustomAlertDialog('エクスポートするデータがありません');
                return;
            }

            // CSV出力用のマッピング (device情報を除く)
